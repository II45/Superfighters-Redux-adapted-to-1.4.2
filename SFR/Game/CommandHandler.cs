    ...
    private static void HandleCommands(ProcessCommandArgs args, GameInfo __instance)
    {
        if (__instance.GameOwner != GameOwnerEnum.Client)
        {
            if (args.HostPrivileges)
            {
                if (args.IsCommand("MOUSE", "M"))
                {
                    if (args.Parameters.Count > 0)
                    {
                        ...
                        args.Feedback.Add(new(args.SenderGameUser, LanguageHelper.GetText("Command.mousecontrol ") + _useHostMouse));
                    }
                }
            ...
            if (args.IsCommand("HELP"))
            {
                ...
                args.Feedback.Add(new(args.SenderGameUser, LanguageHelper.GetText("Command.help"), color, args.SenderGameUser));
                if (args.HostPrivileges)
                {
#if DEBUG
                    args.Feedback.Add(new(args.SenderGameUser, "'/DEBUG [INDEX] [VALUE]' debug purposes", color, args.SenderGameUser));
#endif
                    args.Feedback.Add(new(args.SenderGameUser, LanguageHelper.GetText("Command.help.mouse"), color, args.SenderGameUser));
                }
            }
        }
    }
